...
export default function AddCDNRScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const editId = params.editId ? Number(params.editId) : null;
  
  const { addRecord, updateFullRecord, records } = useCDNRStore();
  ...
  useEffect(() => {
    if (editId) {
      ...
      if (recordToEdit) {
        ...
        setRecipientName(recordToEdit.recipientName || "");
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.container}>
        ...
        <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scrollContent}>
          <View style={styles.card}>
            ...
            <GstinAutoFillInput 
              ...
              onDataFetch={(data) => {
                if (data.purchaserName) setRecipientName(data.purchaserName);
                if (data.state) setState(data.state);
                if (data.supplyType) setSupplyTypeInput(data.supplyType);
              }}
            />
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={styles.inputBox}>
                <InputField value={recipientName} onChangeText={setRecipientName} placeholder="Recipient Name" placeholderTextColor="#9b9b9b" style={styles.inputText} />
              </Input>
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={styles.inputBox}>
            ...
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={[styles.inputBox, { backgroundColor: "#f8fafc" }]} pointerEvents="none">
                <InputField value={state} editable={false} placeholder="State (Auto-filled)" placeholderTextColor="#9b9b9b" style={[styles.inputText, { color: "#475569" }]} />
              </Input>
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={[styles.inputBox, { backgroundColor: "#f8fafc" }]} pointerEvents="none">
                <InputField value={supplyTypeInput} editable={false} placeholder="Supply Type (Auto-filled)" placeholderTextColor="#9b9b9b" style={[styles.inputText, { color: "#475569" }]} />
              </Input>
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={styles.inputBox}>
                <InputField value={noteValue} onChangeText={setNoteValue} placeholder="Total Note Value" placeholderTextColor="#9b9b9b" style={styles.inputText} />
              </Input>
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={styles.inputBox}>
                <InputField value={totalTaxable} onChangeText={setTotalTaxable} placeholder="Taxable Value" placeholderTextColor="#9b9b9b" style={styles.inputText} />
              </Input>
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
        ...
        <Modal visible={noteTypeModalVisible} transparent animationType="fade">
