...
export default function GSTR3B51DetailScreen() {
  const params = useLocalSearchParams();

  const rowId = Number(params.rowId || 1);
  
  const title = params.title || (rowId === 1 ? "Interest" : "Late Fees");

  const [rows, setRows] = useState(getRowsById(rowId));

  const STORAGE_KEY = `GSTR3B51_${rowId}`;
