...
export default function Add5B2BScreen() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const editId = params.editId ? Number(params.editId) : null;
  
  const { addRecord, updateFullRecord, records } = useB2CLargeStore();

  const [state, setState] = useState("");
  const [invoiceNo, setInvoiceNo] = useState("");
  const [supplyType1, setSupplyType1] = useState("Interstate");
  ...
  useEffect(() => {
    if (editId) {
      ...
      if (recordToEdit) {
        ...
        setSupplyType1(recordToEdit.supplyType1 || "");
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.container}>
        <GSTHeader title="5A-B2C (Large Invoices)" />
          ...
          <View style={styles.card}>
            ...
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={[styles.inputBox, { backgroundColor: "#f8fafc" }]} pointerEvents="none">
                <InputField value="Interstate" editable={false} placeholder="Supply Type" style={[styles.inputText, { color: "#475569" }]} />
              </Input>
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
              {Platform.OS === 'web' ? (
            ...
            <FormControl style={{ marginBottom: 16 }}>
              <Input style={[styles.inputBox, (Number(totalInvoiceValue) > 0 && Number(totalInvoiceValue) <= 250000) ? { borderColor: 'red' } : {}]}>
                <InputField value={totalInvoiceValue} onChangeText={setTotalInvoiceValue} placeholder="Total Invoices val" placeholderTextColor="#9b9b9b" style={styles.inputText} keyboardType="numeric" />
              </Input>
              {(Number(totalInvoiceValue) > 0 && Number(totalInvoiceValue) <= 250000) ? (
                <Text style={{ color: 'red', fontSize: fontSizes.sm, marginTop: 4, marginLeft: 4 }}>
                  Invoice value must be more than ₹2,50,000 for B2C Large
                </Text>
              ) : null}
            </FormControl>
            <FormControl style={{ marginBottom: 16 }}>
