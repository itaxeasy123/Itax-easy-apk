...
import { useB2COthersStore, RecordItem } from "../../../store/b2cOthersStore";
import {
  View,
  Text,
  TouchableOpacity,
...
export default function B2COthersScreen() {
  const router = useRouter();
  const { records, deleteRecord } = useB2COthersStore();
  
  const [viewMode, setViewMode] = useState<ViewMode>("primary");
  const [selectedRecord, setSelectedRecord] = useState<RecordItem | null>(null);

  const handleAddRecord = () => {
    router.push("/gst/add-b2c-others");
  };

  const handleEditRecord = (id: number) => {
    router.push({
      pathname: "/gst/add-b2c-others",
      params: { editId: id }
    } as any);
  };
                  ...
                  >
                    ...
                    {viewMode === "primary" ? (
                      <>
                        ...
                        <Text style={[styles.dataCell, { flex: 2, fontWeight: fontWeights.bold }]} numberOfLines={1}>
                          {item.totalTaxable || "0"}
                        </Text>
