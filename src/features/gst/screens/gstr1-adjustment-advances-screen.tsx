...
import { useAdjustmentAdvancesStore, RecordItem } from "../../../store/adjustmentAdvancesStore";
import {
  View,
  Text,
  TouchableOpacity,
...
export default function GSTR1AdjustmentAdvancesScreen() {
  const router = useRouter();
  const { records, deleteRecord } = useAdjustmentAdvancesStore();
  
  const [viewMode, setViewMode] = useState<ViewMode>("primary");
  const [selectedRecord, setSelectedRecord] = useState<RecordItem | null>(null);

  const handleAddRecord = () => {
    router.push("/gst/gstr1-adjustment-advances-add");
  };

  const handleEditRecord = (id: number) => {
    router.push({
      pathname: "/gst/gstr1-adjustment-advances-add",
      params: { editId: id }
    } as any);
  };
              ...
              <View style={styles.tableWrapper}>
                ...
                <View style={styles.headerRow}>
                  ...
                  {viewMode === "primary" ? (
                    <>
                      ...
                      <Text style={[styles.headerCell, { flex: 2 }]}>Taxable Value</Text>
                ...
                {records?.filter(item => item && item.id !== undefined).map((item, index) => (
                  ...
                  >
                    ...
                    {viewMode === "primary" ? (
                      <>
                        <Text style={[styles.dataCell, { flex: 1.5 }]} numberOfLines={1}>{item.place || item.state || "-"}</Text>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.rate || "0"}</Text>
                        <Text style={[styles.dataCell, { flex: 2, fontWeight: fontWeights.bold }]} numberOfLines={1}>{item.totalTaxable || "0"}</Text>
