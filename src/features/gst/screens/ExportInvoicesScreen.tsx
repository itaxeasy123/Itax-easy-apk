...
import { useExportStore, RecordItem } from "../../../store/exportStore";
import {
  View,
  Text,
  TouchableOpacity,
            ...
            <View style={styles.tableContainer}>
              <View style={styles.tableWrapper}>
                ...
                <View style={styles.headerRow}>
                  ...
                  {viewMode === "primary" ? (
                    <>
                      <Text style={[styles.headerCell, { flex: 1.5 }]}>Export Type</Text>
                      <Text style={[styles.headerCell, { flex: 1 }]}>Port Code</Text>
                      <Text style={[styles.headerCell, { flex: 1 }]}>SB No.</Text>
                      <Text style={[styles.headerCell, { flex: 1 }]}>SB Date</Text>
                      <Text style={[styles.headerCell, { flex: 1 }]}>Invoice No.</Text>
                      <Text style={[styles.headerCell, { width: 80, textAlign: 'center' }]}>Actions</Text>
                    </>
                  ) : (
                    <>
                      <Text style={[styles.headerCell, { flex: 1 }]}>Invoice Date</Text>
                      <Text style={[styles.headerCell, { flex: 1.5 }]}>Invoice Value</Text>
                      <Text style={[styles.headerCell, { flex: 1.5 }]}>Taxable Value</Text>
                ...
                {records?.filter(item => item && item.id !== undefined).map((item, index) => (
                  ...
                  >
                    ...
                    {viewMode === "primary" ? (
                      <>
                        <Text style={[styles.dataCell, { flex: 1.5 }]} numberOfLines={1}>{item.exportType?.includes("WPAY") ? "WPAY" : "WOPAY"}</Text>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.portCode || "-"}</Text>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.shippingBillNo || "-"}</Text>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.shippingBillDate || "-"}</Text>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.invoiceNo || "-"}</Text>
                      ...
                      <>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.invoiceDate || "-"}</Text>
                        <Text style={[styles.dataCell, { flex: 1.5, fontWeight: fontWeights.bold }]} numberOfLines={1}>{item.invoiceValue || "0"}</Text>
                        <Text style={[styles.dataCell, { flex: 1.5 }]} numberOfLines={1}>{item.totalInvoiceValue || "0"}</Text>
                        <Text style={[styles.dataCell, { flex: 1 }]} numberOfLines={1}>{item.taxRate || "0"}</Text>
                      </>
                    )}
                  </TouchableOpacity>
                ))}
