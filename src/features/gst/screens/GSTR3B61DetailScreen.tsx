...
export default function GSTR3B61DetailScreen() {
  const params = useLocalSearchParams();

  const rowId = Number(params.rowId || 1);
  const title = params.title !== undefined ? (params.title as string) : "(A) Cash Ledger Balance";
  const subtitle = (params.subtitle as string) || "- Tax";

  const [rows, setRows] = useState(getRowsById(rowId));

  const STORAGE_KEY = `GSTR3B61_${rowId}`;
      ...
      >
        <View style={styles.titleContainer}>
          <Text style={styles.titleText}>
            {title} <Text style={styles.subtitleText}>{subtitle}</Text>
          </Text>
        </View>
